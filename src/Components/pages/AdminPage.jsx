...
function AdminPage({ setAddModal }) {
        ...
        <div className="mt-8">
          {students.map((stundent, index) => (
            ...
            >
              <div class="justify-start items-center gap-[13px] flex">
                <div class="w-[18px] h-[18px] relative">
                  <div className="checkbox-container">
                    <input
                      type="checkbox"
                      id={`checkbox-${stundent._id}`}
                      className="hidden"
                      defaultChecked={isChecked} // используйте defaultChecked для установки начального состояния чекбокса
                      onChange={toggleCheckbox}
                    />

                    <label
                      htmlFor={`checkbox-${stundent._id}`}
                      className={`inline-block w-6 h-6 bg-transparent border border-gray-400 rounded-full ${
                        isChecked ? 'bg-primary' : 'bg-primary' // уберите w-6 h-6 из условия
                      }`}
                    ></label>
                  </div>
                </div>
              ...
              <div class="px-[15px] py-2.5 bg-green-400 rounded-[10px] justify-start items-center flex">
                <div class="text-white text-[15px] font-medium font-['Manrope']">
                  {stundent.searchJob}
                </div>
              </div>
